...
#include "oob-metric-tag.h" // Adaptação para a simulacao deste repositorio https://github.com/jumiso/ecai_arquitetura_oob_lorawan_cps/
...

{
    static TypeId tid = TypeId("ns3::PeriodicSender")
                            ...
                            .AddAttribute("Interval",
                                          ...
                                          MakeTimeChecker()) // <-- Vírgula removida!
                            .AddAttribute ("TagMetrics",
                                        "If true, adds OobMetricTag to each generated packet.",
                                        BooleanValue (false),
                                        MakeBooleanAccessor (&PeriodicSender::m_tagMetrics),
                                        MakeBooleanChecker ())
                            .AddAttribute ("MetricTagValue",
                                        "Value stored in OobMetricTag when TagMetrics is true.",
                                        UintegerValue (1),
                                        MakeUintegerAccessor (&PeriodicSender::m_tagValue),
